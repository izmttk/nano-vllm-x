...
class AttentionMetadata:
    ...
    def build(
        ...
        # Generate page indices for all sequences
        # For each sequence, we need to determine which pages it uses
        paged_kv_indices_list = []
        actual_num_pages_per_seq = []
        kv_start_idx = 0
        
        for _, kv_len in enumerate(kv_seq_lens):
            kv_len = kv_len.item()
            
            # Get the kv_indices for this sequence
            seq_kv_indices = kv_indices[kv_start_idx:kv_start_idx + kv_len]
            
            # Convert token indices to page indices
            # We need the unique page IDs that this sequence uses
            if kv_len > 0:
                page_indices = seq_kv_indices // page_size
                # Get unique pages in order
                unique_pages = torch.unique_consecutive(page_indices)
                paged_kv_indices_list.append(unique_pages)
                actual_num_pages_per_seq.append(len(unique_pages))
            else:
                # Empty sequence, no pages needed
                paged_kv_indices_list.append(torch.tensor([], dtype=torch.long, device=device))
                actual_num_pages_per_seq.append(0)
            
            kv_start_idx += kv_len
        
        # Update paged_kv_indptr with actual page counts
        actual_num_pages_per_seq = torch.tensor(actual_num_pages_per_seq, dtype=torch.long, device=device)
        paged_kv_indptr = torch.cat([
            torch.zeros(1, dtype=torch.long, device=device),
            torch.cumsum(actual_num_pages_per_seq, dim=0)
        ])
        
        # Concatenate all page indices
        if paged_kv_indices_list:
            paged_kv_indices = torch.cat(paged_kv_indices_list)
        else:
            paged_kv_indices = torch.tensor([], dtype=torch.long, device=device)
